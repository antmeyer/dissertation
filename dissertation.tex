...
\begin{abstract}
In this dissertation, I present a multilinear approach to the %\ac{ULM}
unsupervised learning of morphology (ULM), i.e., an approach derived from the multi-tiered (or multilinear) architecture of autosegmental morphological and phonological theory. In motivating this presentation, I isolate the fundamental properties that make the autosegmental formalism so conducive to modeling non-concatenative morphology. I show that these properties are essentially the properties of graph-theoretic terms, an observation that allows me to recast the autosegmental formalism as a graphical model for unsupervised learning, namely the Multiple Cause Mixture Model (MCMM), a bipartite model that is related to the Restricted Boltzmann Machine. The dissertation presents experimental results that support the soundness of this approach. 
