...
In this chapter, I propose a framework for comparing and 
classifying previous approaches to %\ac{ULM}. 
the unsupervised learning of morphology. This framework consists of four categories derived the concepts \emph{nonlinearity} and 
\emph{nonsequentiality} defined in chapter~\ref{ch:intro} (definitions \ref{def:ns} and \ref{def:nl).  We can think of the terms \emph{nonlinear} and \emph{nonsequential} as labels 
for basic categories of models, namely the those that have the property \emph{nonlinearity} and those that have the property \emph{nonsequentiality}, respectively. Recall proposition \ref{prop:nlns}, which posits that \emph{both} nonlinearity and nonsequentiality must be present in a model if it is to be able to handle nonconcatenative morphology. In other words, a model must belong to the \emph{intersection} of the nonlinear and nonsequential models; i.e., it must be a \textbf{nonlinear nonsequential} model.  And indeed, ``nonlinear-nonsequential'' is one of the categories of our four-category framework.  The other three are derived similarly:
...
This one of the four categories that constitute this framework. The other three 
