...
\cite{mccarthy:1981} points out a fundamental flaw in linear, strictly concatenative approaches to morphological analysis: 
Such approaches acknowledge only a single level of representation, namely that of the \emph{segmental string}, i.e., the level of minimal phonological segments (essentially phonemes).\footnote{McCarthy uses the term \emph{segment} in the phonological sense, i.e., a minimal unit of sound. 
Research in \ac{ULM}, however, general use the word to 
refer to minimal morphological units, 
as in \textit{the problem of morphological segmentation}, for example.} 
Because the are restricted to this single layer, the can only represent 
morphological groupings in one way, and that is to insert delimiters of 
some sort 
into the very string under analysis. At any given
insertion point, 
moreover, such an approach can only see only the two segments (or phonemes) immediately to the left and right. It thus
has no way to identify morphs that are made up of discontiguous phonemes.

\cite{mccarthy:1981} offers an alternative \emph{nonlinear} approach to grouping segments into morphs.  that liberates the representation of morphs from the segmental tier. McCarthy's formalism, known as \emph{autosegmental morphology} is based on the theory of autosegmental phonology, developed by \citep{goldsmith:1976} to account for nonlinear phenomena in phonology. One such phenomenon, for instance, is tone spreading, whereby the tone on one vowel spreads to other vowels, leapfrogging any intervening consonants. 
Goldsmith's solution was to introduce \emph{autosegmental} tiers (or planes) of representation that were independent of and external to the segmental tier---tiers that could thus ``see" and access any vowel(s) in the segmental tier, and indeed any subsequence of segments, contiguous or discontiguous. The key was the nonlinearity (or multilinearity)\footnote{The terms nonlinear and multilinear interchangeably in this work, as they both mean, essentially, ``not linear."} introduced by the multiple tiers and their separation both from each other and the segmental tier. 
...
The remainder of this chapter with present a novel framework for comparing and 
classifying previous approaches to ULM . 
...
We thus use the term \textbf{linear} in the sense of `one-dimensional' (i.e., in opposition to \emph{nonlinear). 
...
The following sections will discuss each of these categories in turn, citing examples  sections will use the term
\textbf{nonlinear} (NL) to refer to learning models (or algorithms) one
or more hidden layers to encode the morphological structure.
% in the
