...
RBM activation functions. However, some authors, e.g., \citet{rudy-and-taylor:2014}, have included biases in the activation functions of classical autoencoders.} and $h_k$ and $v_j$ denote preliminary activation values for the a hidden unit and a surface (or visible) unit, respectively.
\begin{align}
\label{eq:sig-m}
...
h_k \,\, &= \,\, p(m_{k}=1 \, | \, \mathbf{r}) \,\, = \,\, \sigma\big(\sum_{j} r_{j} c_{j,k} + \delta_{j}\big) \\
\label{eq:sig-m-sample}
%p_{k} &= \sigma\big(\sum_{j} r_{j} c_{j,k} + \delta_{j}\big) \\
%m_{k} &= \text{rand}_{\textsc{BERN}}\Big(p(m_{k}=1\,\,|\,\,\mathbf{r})\Big)
m_{k}\,\, &= \,\, \text{rand}_{\textsc{BERN}}(h_k)
\end{align}
\begin{align}
\label{eq:sig-r}
v_k \,\, &= \quad p(r_{j}=1 \, | \, \mathbf{m}) \,\, =  \,\, \sigma\big(\sum_{k}  m_{k} c_{k,j} + \theta_{k}\big) \\
\label{eq:sig-r-sample}
%r_{j} &= \text{rand}_{\textsc{BERN}}\Big(p(r_{j}=1\,\,|\,\,\mathbf{m})\Big)
r_{j} \,\, &= \,\, \text{rand}_{\textsc{BERN}}(v_k)
\end{align}
Equations~\eqref{eq:sig-m} and \eqref{eq:sig-r} are basically the same as the classical autoencoder's activation functions. 
However, one of the differences between RBMs and  classical autoencoders is that RBMs require the real-number outputs of the logistic sigmoid function to be mapped to either the 0 or 1.
%; that is, it requires strictly binary hidden and surface units. This is accomplished 
RBMs accomplish this strict boolean mapping by treating each sigmoidal output as  the \emph{probability of getting a successful outcome} from a Bernoulli trial, where a ``successful outcome'' is a valuation of 1 (that is, $m_k = 1$ and $r_j = 1$). Thus, equation~\eqref{eq:sig-m} computes the probability $h_k = p(m_{k}=1|\mathbf{v})$, the probability that the $k$th hidden unit is 1 given the surface-unit activities $\mathbf{r}$. Similarly, \eqref{eq:sig-m} computes $v_k = p(r_{j}=1|\mathbf{h})$, the probability that $j$th surface unit is 1 given the hidden-unit activities. We use $\mathbf{h}$ and $\mathbf{v}$, respectively, to denote the vectors that comprise the individual probabilities
$h_k \text{for} k \in K$ and $v_j \text{for} j \in J$.
...
Evaluate Error: $E = \frac{1}{2} \big( \frac{1}{I\times J} \big) \sum_i \sum_j (r_{i,j} - x_{i,j})^2$\;\label{line:update-E-alt}
      \If{$E_{\text{old}} - E < \epsilon$}{
   Stop\;
   }
}
