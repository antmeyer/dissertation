...
An RBM can almost be described as an autoencoder in bipartite form. There are a few differences, however, as we will soon see.
...
However, one of the differences between RBMs and  classical autoencoders is that RBMs require the real-number outputs of the logistic sigmoid function to be mapped to either 0 or 1 \citep{hinton:2012}.
%; that is, it requires strictly binary hidden and surface units. This is accomplished 
RBMs accomplish this strict boolean mapping by treating each sigmoidal output as  the probability of getting a successful outcome from a Bernoulli trial, where a ``successful outcome'' is a valuation of 1, as in $m_k = 1$ and $r_j = 1$. Thus, equation~\eqref{eq:sig-m} computes $\hat{m}_k = p(m_{k}=1|\mathbf{r})$, i.e., the probability that the $k$th hidden unit is 1 given the surface-unit activities $\mathbf{r}$. Similarly, \eqref{eq:sig-m} computes $\hat{r}_k = p(r_{j}=1|\mathbf{m})$, i.e., the probability that the $j$th surface unit is 1 given the hidden-unit activities. We use $\hat{\mathbf{m}}$ and $\hat{\mathbf{r}}$ to denote the vectors that comprise the individual probabilities
...
$\text{rand}_{\textsc{BERN}}(\hat{m}_k)$ outputs 1 at probability $\hat{m}_k$ (i.e., $\hat{m}_k$\% of the time), and if it 
does not output 1, it outputs 0. Likewise, $\text{rand}_{\textsc{BERN}}(\hat{r}_j)$ outputs 1 at probability $\hat{r}_j$.
...
To obtain the final hidden-unit activations, we pass the vector $\mathbf{\hat{m}} $ to the function $\text{rand}_{\textsc{BERN}}$, which generates a boolean vector by taking a random sample from the probabilities in 
$\mathbf{\hat{m}}$.\footnote{Because the boolean vector in equation \eqref{eq:boolean} is randomly sampled, its values are not deterministic. If we should run $\text{rand}_{\textsc{BERN}}$ again, we might get $[0, 0, 1, 1, 0, 0]$ 
or $[0, 1, 1, 0, 0, 0]$, etc.}
\begin{equation}\label{eq:boolean}
\mathbf{m} \,\,\, =  \,\,\,   \text{rand}_{\textsc{BERN}}(\mathbf{\hat{m}})  \,\,  =  \,\,   [ \, 0, \, 1, \, 1, \, 1, \, 0, \, 0 \,]
\end{equation}
...
%If we should run $\text{rand}_{\textsc{BERN}}$ again, we might get $[0, 0, 1, 1, 0, 0]$ 
