...
and \emph{nonsequentiality} that give rise to this capacity. Furthermore, we saw in 
...
is the vector of hidden units, and $\mathbf{r}_i$ is the layer of surface units. The layer $\mathbf{r}_i$ is actually a  "working" reconstruction of the original surface-unit vector $\mathbf{x}_i$, but we will nonetheless generally refer to it as the \emph{surface layer}, and its components (or nodes) as surface units.\footnote{Saund 
uses $\mathbf{d}$ 
...
%The vector it is the observed, real-world data. 
As a working
...
to relate them to other autoencoders. Section~\ref{sec:mixture-models} then 
...
In the following, we will consider three types
of autoencoders, namely the classical autoencoder, the Restricted Boltzmann Machine (RBM),
and the MCMM. In addition to describing each of these, we will compare them across three dimensions: \emph{form}, \emph{node activation}, 
...
%Equations~\ref{eq:sig-r} and  equation~\eqref{eq:sig-m} are nonetheless very similar to the classical autoencoder's \emph{biases} to weighted sum. These biases are a distinctive feature of RBMs. Like the weights, they are parameters that must be learned
%
...
%That is, \emph{mixing function} is the term that \citet{saund:94} gave to a set of activation functions that have certain
%properties---properties, which, in turn, give MCMMs their distinguishing properties.
%We will discuss the properties of mixing functions in 
...
is assigned to a cluster (distribution) that is responsible for generating it. 
...
Saund thus proposes the following mixing function for the \textsc{write-white-and-black} domain.
 \begin{align}\label{eq:wwb}
  r_{i,j} &=
    ...
    \end{cases}
 \end{align}
% \footnote{\text{Note that it is the \textit{product} $mc$---that is,-1, 0, or 1 in this case---not $m_{i,j}$ or $c_{j,k}$ individually.}}
...
algorithm, in particular, the MCMM. It has advanced the hypthesis 
...
is nonlinear and nonsequential. We have seen the MCMMs meet these criteria. 
...
which will be concerned with the actual testing of the first part’s hypothesis.
 This testing, as we will see, essentially took the form of building of Multimorph, 
 an MCMM-driven system (as we have established the MCMM’s nonlinearity 
 and nonsequentiality in this chapter and chapter~\ref{ch:graph} preceding chapters) 
 and trying it out on Hebrew words, which contained plenty of nonconcatenative morphology. 
 Of course, additional research questions were raised along the way, and we 
