...
the similarities and differences between MCMMs and other types of
 learning algorithms such as Restricted Boltzmann Machines (RBM) \citep{smolensky:1986, hinton:2002, hinton-and-salak:2006}
...
And since MCMMs constitute a definite category, 
...
is the vector of hidden units, and $\mathbf{r}_i$ is the layer of surface units. The layer $\mathbf{r}_i$ is actually a  "working" reconstruction of the original surface-unit vector $\mathbf{x}_i$, but we will nonetheless generally refer to it as the surface layer, and its components (or nodes) as surface units.\footnote{Saund 
uses $\mathbf{d}$ 
...
In particular, we will compare the classical autoencoder, the RBM, and the MCMM across three dimensions, namely \emph{form}, \emph{node activation}, 
(i.e., the means of computing node activities), and \emph{optimization procedure}
(i.e., method for minimizing the model's error). 
Authors whose research involves an autoencoder variant often describe their systems in relation to 
to the classical autoencoder, as do \citet{vincent:2010}, \citet{vincent:2011}, \citet{baldi:2012}, and \citet{chen-and-deng:2016}.
These descriptions and comparisons inform the following discussion's classical-autoencoder portions.
...
For the RBM portions, my main sources were \citet{hinton-and-salak:2006}, \citet{cueto-et-al:2009},  and \citet{fischer:2014}.
...
MCMMs and RBMs, by contrast, both lack an input layer that is distinct from the reconstruction layer. 
...
Thus, in a classical autoencoder (figure~\ref{fig:autoencoder}), the activity of each the hidden node $m_k$ is computed by equation~\eqref{eq:sig-m-ac}. That is, a weighted sum of the input layer's activities is fed to the sigmoid function \citep{bourlard:1988}.
\begin{align}
\label{eq:sig-m-ac}
...
RBM activation functions, although some authors, e.g., \citet{rudy-and-taylor:2014}, have included biases in the activation functions of classical autoencoders. %but the classical encoder could in principle accomodate biases \citep{song:2013}   But the 
The main difference
...
But despite these differences, RBMs and classical autoencoders can be grouped together where activation functions are concerned, since they both (essentially) use the sigmoidal weighted sum.
...
Saund thus proposes the following mixing function for the \textsc{write-white-and-black} domain.
 \begin{align}\label{eq:wwb}
  r_{i,j} &=
    ...
    \end{cases}
 \end{align}
where $-1 \leq c_{j,k} \leq 1$, $0 \leq  m_{i,k} \leq  1$,  and $-1 \leq  r_{i,j} \leq  1$; 
...
These cluster-membership vectors make more sense when we consider the this MCMM's $J \times K$ ($3 \times 2$) weight matrix $\textbf{C}$. The two column-vectors $\textbf{c}_0 = [0,1,0]$ and $\textbf{c}_1 = [1,0,1]$ are the \emph{centroids} of the first and second clusters, respectively. A cluster's centroid is a sort of average or composite of the cluster's member vectors; it represents the feature values that a cluster's members have in common and can thus serve as a label for a cluster. The first data point $\textbf{x}_0 = [0,1,0]$ is a member of the cluster whose centroid is $[0,1,0]$. Similarly, the second data point $\textbf{x}_1 = [1,0,1]$ is clearly a member of the cluster whose centroid is $[1,0,1]$. But just as clearly, the influence of \emph{both} clusters is evident in the third data point $\textbf{x}_2 = [1,1,1]$.
...
% and nonsequentiality in this chapter and chapter~\ref{ch:graph} preceding chapters) 
 and trying it out on Hebrew words, which contain plenty of nonconcatenative morphology. 
