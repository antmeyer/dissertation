...
the $I \times K$ matrix 
$\mathbf{M}$.  Each column in the $\mathbf{M}$ matrix corresponds to a cluster, and thus so does each component $m_{i,k}i$ of a cluster-membership vector. In particular, the $k$th component in a given cluster-membership vector corresponds to the $k$th cluster. It indicates whether or not $i$th data point is a member of that cluster. 
...
which will be concerned with the actual testing of the first part’s hypothesis.
 ...
 will address these in the following chapters. The next chapter discusses the targets of Multimorph's learning, that is, the type of categories one should look for in its output. This is a crucial question for purposes of evaluation.
