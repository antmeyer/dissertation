...
%\textipa{\:t} \textipa{T}%\textipa{\|[t}\textipa{\textltail}
 %ana\textipa{\|[t} \|\:tana\|[t-i\textltail{nca}}
...
%	\end{equation} %\label{eq:sigws
Thus, in a classical autoencoder, the activity of each hidden node $m_k$ is computed by equation~\eqref{eq:sig-m-ac}. 
...
An RBM can almost be described as an autoencoder in bipartite form. 
%There are a few differences, however, as we will soon see.
...
%the require the activities of its hidden and surface units to be strictly 0 or 1 the real-number outputs of the logistic sigmoid function to be mapped to either 0 or 1 \citep{hinton:2012}.
%; that is, it requires strictly binary hidden and surface units. This is accomplished 
%RBMs accomplish this strict boolean mapping by
 RBMs carry out this mapping by interpreting sigmoidal outputs as probabilities and using these probabilities to choose 0 or 1 randomly for each unit. 
 % probabilistically.  views each sigmoidal output as the \emph{probability} that the activity of the unit in question is 1.  
% that the  setting each unit's activity to 1 \emph{randomly} according to the probability yielded my its corresponding (and to 0 according to the complement of this probability).
%RMBs view each sigmoidal output as a \emph{probability}, namely 
...
%Let $\mathbf{\hat{m}}$ and $\mathbf{\hat{r}}$ denote vectors comprising the individual preliminary activities (or probabilities of success)
%$\hat{m}_k$ (for $0 \ge k < K$) and $\hat{r}_j$ (for $0 \ge j < J$), respectively.
%equations~\eqref{eq:sig-m-sample} and \eqref{eq:sig-r-sample} 
%via 
%applying 
These probabilities % in these vectors 
...
$\textsc{Sample}_{\textsc{BERN}}(\hat{m}_k)$ outputs 1 at probability $\hat{m}_k$ (i.e., $\hat{m}_k$ percent of the time), and 0 at probability $1 - \hat{m}_k$.
does not output 1, it outputs 0. Likewise, $\textsc{Sample}_{\textsc{BERN}}(\hat{r}_j)$ outputs 1 at probability $\hat{r}_j$ and 0 at probability $1 - \hat{r}_j$.
%$\textsc{Sample}_{\textsc{BERN}}(v_j)$. which, for each individual probability $h_k$ or $v_j$, randomly chooses 1 at probability   out of the set $\{\,0,\,1\}$
%which randomly chooses $1$ according to it
%the binomial sampling function $\textsc{Sample}_{\textsc{BERN}}$, which samples outcomes from a binomial distribution.
...
%One of the ways in which an RBM differs from a classical autoencoder is that it requires the real-number outputs of the logistic sigmoid function to be mapped to either 0 or 1. This is accomplished in equations~\eqref{eq:sig-m-sample} and \eqref{eq:sig-r-sample} via the binomial sampling function $\textsc{Sample}_{\textsc{BINOM}}$, which takes two parameters, namely the input $n$, the number of the trials, and the (conditional) probability that a given trial should yield a ``successful outcome'' (i.e., a value of 1 for the variable in question). 
Suppose, for example, that the logistic sigmoid \eqref{eq:sig-m} output the following preliminary activities for a hidden-unit vector.
\begin{equation}
...
%These values are outputs of the logistic sigmoid equation \eqref{eq:sig-m}.
...
$\mathbf{\hat{m}}$.\footnote{Because the boolean vector in equation \eqref{eq:boolean} is randomly sampled, its values are not deterministic. If we should run $\textsc{Sample}_{\textsc{BERN}}$ again, we might get $[0, 0, 1, 1, 0, 0]$ 
or $[0, 1, 1, 0, 0, 0]$, etc.}
\begin{equation}\label{eq:boolean}
\mathbf{m} \,\,\, =  \,\,\,   \textsc{Sample}_{\textsc{BERN}}(\mathbf{\hat{m}})  \,\,  =  \,\,   [ \, 0, \, 1, \, 1, \, 1, \, 0, \, 0 \,]
\end{equation}
%Because the boolean vector in equation \eqref{eq:boolean} is randomly sampled, its values are not deterministic.
%Note, however, that each component in $\mathbf{m}$ is \emph{randomly} sampled given the probabilities in 
%$\mathbf{\hat{m}}$. These values are thus not deterministic.
...
%If we should run $\textsc{Sample}_{\textsc{BERN}}$ again, we might get $[0, 0, 1, 1, 0, 0]$ 
%or $[0, 1, 1, 0, 0, 0]$, etc.  %Each component is randomly sampled given the probabilities in $\mathbf{h}$.
...
the RBM and classical autoencoder is that the RBM's activation functions~\eqref{eq:sig-m} and \eqref{eq:sig-r} 
both refer to the same weight matrix $\textbf{C}$, whereas those of the classical 
autoencoder (equations~\eqref{eq:sig-m-ac} and \eqref{eq:sig-r-ac}) each 
refer to a different weight matrix 
($\textbf{A}$ in equation~\eqref{eq:sig-m-ac} and $\textbf{B}$ in equation~\eqref{eq:sig-r-ac}). 
An RBM's learning process is thus bidirectional: 
...
activities are conveyed back to the hidden units via the \emph{same} weight matrix, resulting in a cyclic process.
...
% 	\def \china{0cm}
 	...
 	\node[label]	(ml00) 	at (1.45cm,\attic)		{$\large m_{0,0}$};
