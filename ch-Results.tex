...
a qualitative component and a dual-paradigm quantitative component, i.e., a quantitative component with 
\emph{intrinsic} and \emph{extrinsic} sub-components. 


Recall that Multimorph's output, in its raw form,
...
data points (see chapter~\ref{ch:MMMM}).} While these are numerical matrices,
...
algorithm~\ref{alg:members}, and clusterings then evaluated according to the diverse 
...
%eaklt \> awklt \> blklwkim \> awkl \> awtkm \> eawklt \\
%eakltm \> eklblbim \> hawkl \> elhakil \> bawkl \> enwakl \\
%hkwl \> hkil \> akltm \> blklkh \> hmkxwl \> blklkw \\
%hklim \> cvrk \> ekwlm \> bekl \> aklti \> akl \\
%hmikl \> enstkl \> blklk \> htlklkh \> astkl \> haklt 
\end{tabbing}
\end{normalsize}
\vspace{3pt}
\begin{mdframed}
...
%\texttt{d<\a'{i}} \> (1.0000) \> \texttt{d<l} \> (1.0000) \> \texttt{l<q} \> (0.8525) \> \texttt{d<q} \> (0.7975) \> \texttt{\a'{i}<q} \> (0.5013)\\
%\texttt{h<d} \> (0.3741) \> \texttt{l<\a'{i}} \> (0.1622) \> \texttt{g<d} \> (0.1572) \> \texttt{\a'{e}}@\texttt{[-1]} \> (0.1353) \> \texttt{r}@\texttt{[-2]} \> (0.1347)
\end{tabbing}
\end{small}
\end{mdframed}
\vspace{-5pt}
...
Recall that
algorithm~\ref{alg:members} (section~\ref{sec:instrinsic}) converted the MCMM's $\mathbf{M}$ and $\mathbf{C}$
...
$\langle s=4, \delta=3\rangle$ clustering, for instance, the first cluster (i.e., the cluster indexed 0) corresponded to the morph \textit{-\'{e}\_et}, 
...
also tended to occur at higher cluster indices; that is, the 
MCMM tended to find them later during the learning process. 
Individual roots to not occur in as many distinct words
as concatenative morphs and vowel patterns, since roots are entirely lexical in nature. Root clusters thus tend to be among the smallest of clusters.

There were in root clusters
 in the clusterings of all three data types, but the
but orthographic clusterings contained the most root clusters. This is perhaps to some degree a result
of the prevalence of consonants in Hebrew orthography (see chapter~\ref{ch:experi}).
It does not represent many vowels, and thus the consonants tend to be closer to each other 
and are often adjacent. Thus, precedence features in the orthographic data can generally 
take in more consonants at once than in the transcriptional datasets. 
...
and, second, the ten most active features in the cluster's centroid. 
The first four figures all give samples of  clusters corresponding to the root \textit{\textipa{P}.k.l}; 

Figures~\ref{fig:cluster-182-1-3-O} shows a word sample from a cluster corresponding to a root, namely \textit{\textipa{P}.k.l}, which appears in words pertaining to eating, for example, %\textit{\textipa{P}o\textsubdot{k}\'el\} `he/it is eating,'
\textit{\textipa{P}a\textsubdot{k}\'al} `he/it ate' and \textit{\textipa{P}a\textsubdot{k}il\'a} `eating.' 
This is cluster $k = 182$,   
in the clustering $\langle s=1, \delta=3 \rangle$. Its data source was the orthographic data set. Its words are expressed in the (transliterated) orthographic alphabet (see the discussion in chapter~\ref{ch:experi}. Thus, \textit{\textipa{P}.k.l} becomes \textit{a.k.l}, for instance.
%generated at the parameter settings $\langel s=1$ $\delta=3 \rangle$. 
In the words that actually contain the root \textit{a.k.l}, the letters \textbf{a}, \textbf{k}, and \textbf{l} appear in boldface.
Notice that not all of the words have this root.

Figures~\ref{fig:cluster-91-3-2-O} and \ref{fig:cluster-81-2-2-O} correspond to \textit{a.k.l}
(i.e., \textit{\textipa{P}.k.l}) clusters from other clusterings, namely those generated at
$\langle s=3, \delta=2 \rangle$ and $\langle s=2, \delta=2 \rangle$, respectively. 
Notice that each of these clusters is better than the previous one in that each is 
a \emph{purer} representation of the root \textit{a.k.l} (= \textit{\textipa{P}.k.l}); that is,
each has a greater proportion of words that actually contain \textit{a.k.l}.
Furthermore, observe that the two \emph{most} active feature for each of these clusters are
\begin{center}
...
encoding the nonconcatenative sequence \textit{a.k.l}.
In figure~\ref{fig:cluster-182-1-3-O}, the activities of \texttt{k<l} and \texttt{a<k} are 1.0 and 0.8271, respectively.
Figure~\ref{fig:cluster-91-3-2-O}, showing purer cluster, has these features at activities 1.0 and 1.0. It also has \texttt{a<l} among its top ten features, 
...
its ten,
and, moreover, all with a perfect activity of 1.0.
...
having come from the transcriptional dataset with accent-marking. This cluster was generated under the parameter settings $\langle s=0, \delta=2 \rangle$. Recall from chapter~\ref{ch:experi} that an $s$ value of 0 means that were no positional features. 
...
Figure~\ref{fig:cluster-33-0-2-TS} shows partial success in finding the root 
\textit{\textipa{P}.k.l}, though less than the preceding three figures. This is
 probably not due to the absence of positional features, however, since positional 
 features do not seem conducing to representing a nonconcatenative sequence of 
 symbols such as a Hebrew root. The boldface letters in figure~\ref{fig:cluster-33-0-2-TS} 
 demonstrate the positional variability of root consonants from word to word. 
 Positional features are thus likely not to be of much help in representing a root. 
 The mere addition of a prefixes changes the root characters' positions from \textit{\textbf{\textipa{P}}a\textbf{\textsubdot{k}}\a'{a}\textbf{l}} to \textit{we\textbf{\textipa{P}}a\textbf{\textsubdot{k}}\a'{a}\textbf{l}}. 
 But the positions of the root consonants relative to each other can also vary, 
 as does the position of the \textit{l} relative to the \textit{x} in \textit{we\textbf{\textipa{P}}a\textbf{\textsubdot{k}}\a'{a}\textbf{l}} and \textit{we\textbf{\textipa{P}}a\textbf{\textsubdot{k}}\textbf{l}\a'{u}}. 
...
Let's take a closer look at the features of this bad boy.
Do we see anything curious?
This cluster's centroid features are not ideal for the root \textit{\textipa{P}.k.l}. Note the activities of the features 
{\textipa{P}<\textsubdot{k}} (= orthographic \texttt{a<k}) and \texttt{\textsubdot{k}<l} (= orthographic \texttt{k<l}):  0.4984 and
 0.7588, respectively.  Notice that many of the features reference a vowel. 
...
In fact, the two most active features, namely \texttt{a<\textsubdot{k}} (activity 1.0) and \texttt{\textsubdot{k}<\'{a}} (activity 0.8585), both reference a vowel. (In the transcriptional datasets, \textsf{a} is a vowel.) References to particular vowels are problematic for roots because the vowels that interleave with a root's consonants vary from word to word. 

Another cluster of transcribed words is shown in figure~\ref{fig:cluster-339-4-3-TS}. 
...
at 1.0 and 0.853. We also have the feature \texttt{d<q} with the activity 0.798. Such features spanning 
the first to the third root consonant are likely to be rarer and/or weaker in the transcriptional domain, 
...
as a modest activity of 0.501. The \textit{i} in these to features is reflected in frequency of \textit{i} in 
this cluster's words. It would thus be more accurate to say that this cluster represents subset of the 
...
Multimorph's capacity to learn vowel patterns. The pattern in this case is \textit{a.\'{e}}.
 It is a morphological component of \textit{Hitpa`el} binyan, 
as in \textit{mitrax\a'{e}c},  
the prefix stem of the \textit{Pi`el},
...
In general, the precedent feature type seems to be essential for representative 
nonconcatenative phenomena of any kind. 
Positional features, by contrast, are not essential for representing nonconcatenative morphs, and could be 
detrimental.
...
\subtable[Orthographic (O), $K=500$\label{subtab:intr-O-500}]{
\footnotesize
\centering
\setlength{\extrarowheight}{3pt}
\begin{tabular}{cc|ccccrr}
...
One of the most salient observations regarding tables~\ref{tab:intr-500} and \ref{tab:intr-1000}
 is that the models trained on O, the orthographic dataset, performed just as well as as those trained on TS, the transcriptional datasets with stress marking, if not \emph{slightly better}. The average F-score for all O 
 models at $K=1000$ (that is, the average F-score of \emph{all} valuations of $s$
   and $\delta$ in table~\ref{subtab:intr-O-1000}) was 0.371.
...
\emph{First, the O models performed surprisingly well.} 
They performed as well as the TS models and substantially better than the
  TR models.
...
vowels whose stress is not specified.  It is important to note, however, that the 
alphabets used in this study were consisted solely of atomic unicode characters. 
...
are completely distinct, as (un)related as the characters \textit{a} and \textit{t} as far as the unicode is concerned.
There is likewise
...
We also must note the precipitous drop in coverage in the orthographic models. In table~\ref{subtab:intr-O-500}, the coverage drops from 11051 to 4488 precisely when $s=0$ becomes $s=1$. Table~\ref{subtab:intr-O-1000} shows essentially the same drop and at precisely the same $s$ value (i.e., affix length).  In both of these tables, the coverage decreases even further as $s$ increases to $s=3$.  %low at $s=2$ and $s=3$, even decreasing further, by another 1000 words, in fact. 
We do not see this decrease in the transcriptional models. %Apparently the introduction of positional features  
At the same time, $K^{\prime}$, the number of clusters with at least one active member, is maximized in the orthographic models at $s = 1$, precisely when the coverage is lowest, indicating a greater degree overlap between the clusters. Perhaps the positional features encourage the MCMM to pay more attention to words containing positional regularities, as in cases of frequent affixes.  %highly frequent suffix or beginning with a highly frequent prefix. %($K=500$)
Positional features are intrinsically more specific than precedence features. Precedence features specify a pair of characters, but not the particular \emph{indices} of these characters within the word. Depending on $\delta$. The distance between the characters is not even specified for most values of $\delta$ (see section~\ref{sec:features}).
...
Because of their specificity, positional features perhaps make it generally harder for a word to gain membership in a cluster, hence he abrupt decrease in coverage once positional features are introduced. The reason we see this dramatic decrease only in the orthographic data could be due to the following factors: 
There are lot more precedence features in the transcriptional feature sets than in the orthographic feature set. The number of precedence features for a particular alphabet is the size of the alphabet \emph{squared}. For example, the size of the orthographic alphabet is 22, so the number of orthographic precedence features is $22^2 = 484$. By contrast, there are 49 characters in the TS alphabet is 39 and thus $39^2 = 1521$.\footnote{Features that never occurred emong the actual words of the dataset in question dataset were deleted, so the actual numbers were somewhat lower than these.}
...
The size of the positional feature set increases linearly with the size of the alphabet, while that of of the precedence increases exponentially 
...
Because of their specificity, positional features might make it harder for a word to gain membership in a cluster. At the edges of words, this increased specificity seems to have had a positive effect, as \dots
whereas F-scores tend to be lower where $s = 2$ and $s=3$. Moreover, the coverage in the orthographic clusters decreased even further at $s$
...
The highest F-scores in all three data types tend to occur at rather low $\delta$ values , suggesting that positional features are only useful at the edges of words (see section~\ref{sec:concl-features} in the next chapter).
...
The vast array of precedence features in the transcriptional features (especially TS) perhaps drowns out the influence of the relatively scarce transcriptional positional features. In the TS feature sets, the precedence to positional ratio was 9.7 to one. In the orthographic feature sets, by contrast, there was 5.5 precedence for every one positional feature.
...
Morfessor, the idea being to give Morfessor a pre-analyzed (or pre-digested, as it were) list of input words and
 ...
 the control group, which was in this case the original Hebrew wordlists, untouched by Multimorph. Because each
experimental model, i.e., each combination of $s$ and $\delta$ values, had different word coverage---i.e., involved a somewhat different set of words---each required its own control model. Thus, table~\ref{tab:extr-results-1000} shows control results for each valuation of $s$ and $\delta$.
...
experimental models. One must bear in mind that this four-stage process is both new---it was executed for the first time in this study---and highly complex. The process's results in table~\ref{tab:extr-results-1000} are close enough to 
...
\section{Summary}
To sum up, in the qualitative analysis, we found clusters corresponding to roots, vowel patterns, as well as concatenative affixes. The orthographic (O) models produced more corresponding to consonantal roots than the transcriptional models (TS and TR), and in general, produced root clusters that were more coherent as well. But since the orthographic representation favors consonants at the expense of vowels (see chapter~\ref{ch:experi}), this is perhaps was to be expected. We also saw all types of models represented nonconcatenative phenomena through precedence features, and that at least two precedence were necessary to represent a tri-consonantal root, whereas one precedence feature sufficed to represent most vowel patterns.


According the intrinsic results the O models performed at a level on par with the TS models. In the extrinsic results, the orthographic models were the best, coming closest to matching those of the control model. 
