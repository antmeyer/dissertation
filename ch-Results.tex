...
$\langle s=4, \delta=3\rangle$ clustering, for instance, the first cluster (i.e., the cluster indexed 0) corresponded to the morph \textit{-\'{e}\_et}.\footnote{The \_ here represents a single slot occupied by a consonant of a \emph{different} morph, namely the root. The \_  thus indicates discontiguity.}
...
% shows a word sample from a cluster corresponding to a root, namely \textit{\textipa{P}.k.l}, which appears in words pertaining to eating, for example, %\textit{\textipa{P}o\textsubdot{k}\'el\} `he/it is eating,'
...
(\textit{\textipa{P}.k.l}) clusters from other clusterings, namely the clusterings generated at
...
Figure~\ref{fig:cluster-91-3-2-O}, showing a purer cluster, has these features at activities 1.0 and 1.0. It also has \texttt{a<l} among its top ten features, 
...
\textit{\textipa{P}.k.l}, though less than the preceding three figures. This is
 ...
 The mere addition of a prefix changes a root characters' positions, as it does when \textit{we-} is added to \textit{\textbf{\textipa{P}}a\textbf{\textsubdot{k}}\a'{a}\textbf{l}} to yield \textit{we\textbf{\textipa{P}}a\textbf{\textsubdot{k}}\a'{a}\textbf{l}}. 
 But the positions of root consonants can also vary \textit{relative to each other}.  
...
Taking a closer look at this cluster's top ten centroid features, it becomes clear they are not ideal for capturing the root \textit{\textipa{P}.k.l}. Note the activities of the features 
...
Positional features are intrinsically more specific than precedence features. Precedence features each specify a pair of characters, but not the particular \emph{indices} of these characters within the word. The distance between the characters is not even specified for most values of $\delta$ (see section~\ref{sec:features}).
