...
The sequence \textit{a.\'{a}} also serves as a \emph{mishkal}, that is, a distinctive pattern for a class of nouns. 
%\textit{a.\'{a}}
In particular, \textit{a.\'{a}} (or CaCaC) represents a class consisting (mostly) of profession words \citep{ravid-and-schiff:2006, shatil:2014}, e.g., \textit{nag\'{a}n} (`musician'),  \textit{kat\'{a}v} (`writer, journalist'), \textit{\v{s}ar\'{a}t} (`janitor'), 
\textit{\textsubdot{t}ab\'{a}x} (`chef, cook'), \textit{nag\'{a}r} (`carpenter').\footnote{This pattern, though a nominal pattern, is related to the \textit{Pi`el} binyan (or verbal pattern). In earlier forms of Hebrew, the \textit{Pi`el} binyan called for the gemination of the second root consonant (see the discussion in \citet{mccarthy:1981:conslength}). Because the CaCaC nominal pattern was related to the \textit{Pi`el} binyan, it required this gemination also, and thus its historical form was *$\text{C}_1$a$\text{C}_2$$\text{C}_2$a$\text{C}_3$. However, since geminate consonants do not exist in Modern Hebrew, its modern form is $\text{C}_1$a$\text{C}_2$a$\text{C}_3$ \citep[see][]{bolozky-and-schwarzwald:1990, bolozky:1997}.}
%(The \textit{Pi`el} 3rd-persion masculine singular pattern was $C_1$i$C_2$$C_2$e$C_3$
%$C_1$a$\text{C}_2$a$\text{C}_3$. \citep{bolozky-and-schwarzwald:1990, bolozky:1997}}
