...
\textit{\textsubdot{t}ab\'{a}x} (`chef, cook').\footnote{This class primarily consists of words for professions. It is actually related to the \textit{Pi'el} binyan; in past forms of Hebrew, the second root consonant would have doubled, yielding \textit{nagg\'{a}n}, \textit{\textsubdot{t}abb\'{a}x}, etc. The historical pattern was thus CaCC\textipa{"\r{a}}C, the first \textit{a} being short and protected from reduction by the closed syllable.   There is in fact another \textit{a.\'{a}} noun class---this one related to the \textit{Qal} (or \textit{Pa`al}) binyan. Historically, its pattern was C\textipa{\r{a}}CC\textipa{"\r{a}}C, with an initial long rounded \textit{a} (i.e., \textipa{\r{a}}). Today, these two nominal vowel patterns have the same pronunciation; i.e., both are pronounced CaC\'{a}C.}
