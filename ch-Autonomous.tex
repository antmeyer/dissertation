...
%\begin{itemize}
%\item 
yo demonstrate the legitimacy of autonomous morphology as a learning target for a \ac{ULM} system, i.e., to show that it makes sense to try to learn morphological units that are not morphemes.
%\item 
We will discuss some of the more prominent theories of autonomous morphology, noting their significance to \ac{ULM} (and Multimorph in particular.) 
...
(\ac{ULM}), authors often use the word \emph{morphemes} 
...
units. We shall further discuss the notion of autonomous morphology in section (REF). %, but for now, observe 
...
A \ac{ULM} system might learn subsequences of word strings that tend to resemble
 classical morphemes \emph{in form}, but even in such cases, \ac{ULM} systems are oblivious to what
...
computational linguists, including researchers in the field of \ac{ULM}, tend to presume %a morpheme-based morphology.
...
and $\sigma\text{:}\{ \textsc{pers:sg}\text{,} \, \textsc{num:sg}\text{,} \, \textsc{tns:pres} \}$ is an \ac{MPS}.
...
by an identity operation, from forms of the past tense, such as opened in They opened it."
 [**Insert Example Here.**]
...
%  extends the notation of realizational morphology \citep{stump:2001} to handle derivational morphomes. 
  ...
  One of Round's contributions is his formalized descriptions of morphomic phenomena. In \cite{round:2011}, for instance, he expresses the \emph{morphomic representation} of a word \textit{WORD} as a function $ \mu{\textsc{word}}$ that takes as input a pair of items, the precise nature %follows:
  of which depends on whether one is dealing with inflectional or derivational morphology. In the former case, the input pair comprises a lexeme $\lambda$ and an \ac{MPS} ($\sigma$); in the latter, it consists of a root $\rho$ and a derivational property set $\delta$. Both cases are presented in table~\ref{tab:morphreps}.
% \begin{align}
...
Consider the morphomic 
representation       %, that sameness at the surface level indicates sameness at the morphomic level.
$\mu{\textsc{word}}(\lambda,\sigma)$. %Suppose now that we were to replace 
Suppose the \ac{MPS} $\sigma$ were replaced with a different \ac{MCU}, $\tau$. %, such that $\sigma \ne \tau$. 
This change could result in a different phonological realization, but not necessarily. 
%not necessarily change the surface, or phonological, form. %That is, 
That is, $\Phi{\textsc{word}}(\lambda,\tau)$ could very well be identical to 
 ...
 present-tense verbs. See, for example, table~\ref{tab:engverbpres}.
Every form in the present-tense paradigm is the same except for the 3sg, which takes the \textit{-s} suffix  %for a given lexeme contain the same form, namely the bare form of the verb. %English verbs are realized as the same formbare form of the verb. 
...
A central maxim of morphomic theory is that \emph{sameness at the phonological level entails sameness
...
%Suppose also that for both \ac{MPS}s ($\sigma$ and $\tau$), and that we are dealing 
%Then, $(\lambda, \sigma)$  and $(\lambda, \tau)$ would map 
%Then, $(\lambda, \sigma)$ and $(\lambda, \tau)$.
%These two $\langle$ lexeme, \ac{MPS} $\rangle$ pairs are certainly different, but they nonetheless 
%correspond to the \emph{same} phonological form:
...
Round's taxonomy comprises three categories, namely %according to the %scope of the morphomic relation and
%the nature of the the \emph{shared aspect of form} that defines the morphome in question. %of the shared element of ththat defines the relation. 
...
to a class of 
forms whose members 
can serve certain a variety of  syntactic purposes. For instance, it can be combined
...
\ref{ex:i-and-t}: %and 2 and arrive at  
...
% as the the phonological forms \textit{-t}, \textit{-i}, and \textit{-it} 
