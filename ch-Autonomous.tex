...
\end{align}
The subscript $V$ in \eqref{eq:roexp} denotes the lexical category of \textit{X}, which is %a global property of the paradigm and  
in this case is \textsc{verb}, 
and $\sigma$ and $\tau$ are \ac{MPS}s. %\text{:}\{ \textsc{pers:sg}\text{,} \, \textsc{num:sg}\text{,} \, \textsc{tns:pres} \}$ is an \ac{MPS}.
...
C its lexical category, and $\sigma$ its MPS. However, instead of the usual $f(\text{X})$ %sort of morphological operation 
...
% perfect participles by taking the  
...
Suppose the \ac{MPS} $\sigma$ were replaced with a different \ac{MCU}, namely $\tau$. %, such that $\sigma \ne \tau$. 
...
That is, $\Phi{\textsc{word}}(\lambda,\tau)$ could very well be identical to 
 ...
 present-tense verbs, as exemplified in table~\ref{tab:engverbpres}. Every form is identical except the 3.sg, which takes the \textit{-s} suffix.%for a given lexeme contain the same form, namely the bare form of the verb. %English verbs are realized as the same formbare form of the verb. 
...
A central maxim of morphomic theory is that \emph{identity at the phonological level entails identity
