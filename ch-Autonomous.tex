...
equivalence signified, nor the semantic 
...
\cite{stump:2001} derives four categories of morphological theory. 
...
In particular, $\text{L}$ is a lexeme, and $\text{X}$ is a stem, i.e., the form correspondent of $\text{L}$. 
...
a theory of an \emph{autonomous} layer of morphology, a layer responsible 
...
\subsubsection{A Taxonomy of Morphomes}\label{sec:round-taxonomy}
Another contribution of Round is his taxonomy of morphomes, which, for our purposes, will be
useful for discussing the limitations on unsupervised morphological learning, i.e., what a ULM 
system can and cannot learn given the nature of its input. The taxonomy
classifies morphomes according to the types of equivalence classes they represent. 
Essentially, each category in this taxonomy represents a different approach to relating forms to other forms; each is a different set of standards or conditions for placing forms in the same class. %--orms can be considered ``the same.''
...
The taxonomy has three categories:
...
By contrast, both rhizomorphomes and metamorphomes transcend individual, specific lexemes. A single metamorphome incorporates many lexemes, as does a single rhizomorphome.
%eas we will see below.% both comprise multiple %the confines of 
%particular lexemes and are thus both
%rather abstract. 
Of these three, metamorphomes and meromorphomes are the most 
...
\centering
   \subfigure[{[{\textglotstop}{avavit}]} `spring-like'\label{subtab:heb-avivit}]{
    ...
    \vspace{6pt}
      \subfigure[{[xalalit]} `spaceship'\label{subtab:heb-xalalit}]{
     \centering
     \setlength{\extrarowheight}{8pt}
    \begin{tabular}{l c c}
       \toprule
...
That is, following \citet{faust:2013}, we shall
...
\section{Implications for ULM and Multimorph}\label{sec:morphomic-implications}
