...
In the literature on the Unsupervised Learning of Morphology 
(ULM), authors often use the word \emph{morphemes} 
...
when they they use the word \emph{morphology}, even though there
are currently at least two fundamentally different views regarding \emph{what morphology is}, namely
the morpheme-based and lexeme-based views, to borrow the labels that \cite{aronoff:1994} has assigned 
to these categories. The lexeme-based view, to which a majority of morphologists now subscribe (CITE),
maintain that there exists an \emph{autonomous} layer of layer of morphology, an layer that mediates between (morpho-)syntax
and phonologically realized form, but has neither meaning nor phonologically realized form in and of itself. Proponents
of lexeme-based morphology thus do not believe in pairings of form and meaning, i.e., morphemes , as fundamental linguistic
units. We shall further discuss the notion of autonomous morphology in section (REF) below.

%At this point, the attentive reader would be well-justified 
%in asking, 
%``WTF? What the F-word does Morfessor learn? What indeed 
%does \emph{any} 
%ULM system learn?" Indeed, dear reader, these are 
%excellent questions; 
...
Now, the output of any given machine learning system depends to 
a great extent on the nature of the input, which, in the case of the \emph{unsupervised} learning of morphology is concerned, 
has a lot to do with the meaning of ``unsupervised." 
%In the case of ULM, the input is invariably a list of contextless words [cite?]. 
...
%Now unsupervised learning is of course central to ULM, 
...
case ULM, the meaning of \emph{unsupervised} includes a
...
not aware of a ULM system that extracts features from a word's raw context, i.e., form
unanalyzed adjacent words. Presumably, this is because 
such context would not be helpful to a machine learner. 
...
Meaning is one half of a classical morpheme,
but in order to deduce meaning, one needs contextual information. Because
a ULM system does not have access to context, it simply cannot learn classical morphemes.
A ULM system might learn subsequences of word strings that tend to resemble
 classical morphemes \emph{in form}, but even if in such cases, ULM systems are oblivious to what
 these pieces of form actually mean. 
Since the words under analysis in ULM are not accompanied by contextual features,
there is no way to induce classical morphemes in ULM,
...
And thus, ULM systems, particularly those which learn solely from 
...
of this of equivalence class. We might
guess that it is morphosyntactic category of \emph{some kind}, but since we would be ignorant
...
clear that \textsc{null}\textit{.ed.ing.s} are suffixes that attach to verbs. 
%[These stems were verbs clearly the nullsoft facts attached to the base case the e d direct past tense the ing corresponds to the progressive or the present participle in the s the third person singular present tense suffix.] 
Now, it is not hard to guess the lexical category of the stems that accompany 
\textsc{null}\textit{.ed.ing.s}. Clearly these are verbal suffixes.
But we know this only because we know that \textsc{verb} + \textit{-ed} realizes a
...
sets of stems that combine with the same affixes (and sets of affixes that combine with with the same stems) %and sets
%of affsuch that  all of the stems comacombine with the same affixes and vice versa, 
it does not learn to associate \textit{-s}, for example, 
with `3rd-person singular present-tense'. That is, does not assign particular
meanings to the stems and affixes it discovers.
%the \textit{-s} of the signature \textsc{null}.\textit{s} 
%most likely realizes plural number when
%combined with nouns. 
...
phenomena. Morphemes, therefore, are not reasonable targets of unsupervised learning.
...
%but in any case, the output of any ULM system will consist of forms entirely or almost entirely and be virtually void 
%of any real meaning. 
...
However, it does not learn morphemes. The same can be said of Morfessor, the system of \cite{poon-et-al:2009}, and %virtually
...
over the last 1.2 million years. %are two opposing views in morphology. 
...
\begin{xlist}
	\ex \textbf{Lexical}: The meaning of \textit{issue-s} is determined by the lexical entries of its components \textit{issue} and \textit{-s}.
...
\begin{xlist} 
	\ex \textbf{Incremental}: Each affix has its own intrinsic \emph{local} meaning. 
...
as the seat of meaning. %These are the legacy of Bloomfield and the descriptivists.
...
%(also known as American structuralists), 
%a school of linguistic 
%thought and methodology whose influence is still strongly felt today. 
%\cite[][p. 161]{bloomfield:1933} 
%famously defined a morpheme as
%a ``linguistic form which bears no partial phonetic-semantic resemblance to any other form," i.e., 
It was \cite[][p. 161]{bloomfield:1933} who famously wrote,``A linguistic form 
which bears no partial phonetic-semantic resemblance to any other form is a \emph{simple} form or \emph{morpheme}" (emphasis in the original).%\citep[][p. 161]{bloomfield:1933}.
...
the undergraduate and graduate level. Any introductory text containing a unit on morphological analysis is going to present a methodology that is essentially Bloomfieldian \cite{anderson:2017}.
...
computational linguists, including researchers in the field of ULM, tend to presume %a morpheme-based morphology.
...
The MPSs associated with content cell and form cell are $\sigma$ and 
...
the form-paradigm column in table \ref{tab:engverbpres}. %namely  \{\textsc{p}:{1}$\vee${2}, \textsc{n}:{sg}$\vee${pl}\} (\textit{kick}) and 
%\textbf{\textsc{p}:{3}}, \textbf{\textsc{n}:{sg}} (\textit{kicks}). 
...
a distinct corresponding phonological form. The %realizational rules (of the phonological level) are not linked directly to the 
...
Realizational rules come in come in two varieties:
\emph{rules of exponence} and \emph{rules of referral}. Rules of exponence, e.g., \eqref{eq:roexp}, describe the 
realization of surface forms from morphological \emph{exponents}.
\begin{align}
\label{eq:roexp}
	\text{X}_{V},\sigma\text{:}\{ \textsc{pers:3}\text{,} \, \textsc{num:}\text{sg,} \, \textsc{tns:}\text{pres} \} &= \text{X}s \\
	\text{X}_{V},\sigma\text{:}\{((\textsc{pers:}\text{1}\vee\text{2}) \land (\textsc{num:}\text{sg}\vee\text{pl})) \vee (\textsc{pers:}3 \vee \text{sg})\} &= \text{X}
\end{align}
where the subscript $V$ denotes the lexical category in question, which is a global property of the paradigm and  in this case is \textsc{verb}, 
and $\sigma\text{:}\{ \textsc{pers:sg}\text{,} \, \textsc{num:sg}\text{,} \, \textsc{tns:pres} \}$ is an MPS.
By contrast, a rule of referral redirects the word formation process to a different paradigm. 
...
as well as a %\emph{function} that conveys a
``mapping from morphosyntax to phonological realization" \citep[][p. 25]{aronoff:1994}. It is related to 
...
\cite{aronoff:1994} and the concept of the 
...
%(clarify, formalize) the nature of the morphome.
...
%ecause it his both a practical application of a theoretical  morphome-based as well as  have contributed a great deal to the formalizing and filling out the concept of the morphome. I for one have found his work to be very helpful %This wor a major contribution to language documentation.
\cite{round:2011} expresses a word's \emph{morphomic representation}
 function of a pair consisting of particular lexical item (or index to such an item) 
 and a particular set of properties. In a case of inflectional morphology, the 
 first item is a \emph{lexeme} $\lambda$ (or lexical index) and the second a 
 morpho-syntactic propery set $\sigma$. However, where derivational morphology 
 is concerned, the first item is a \emph{root} ($\rho$), and the second a 
derivational property set ($\delta$).
 This gives us the morphomic representations in \ref{tab:morphreps}.
...
%Derivation & $\langle \delta,\rho \rangle$ & $\mu{\text{WORD}}(\delta,\rho)$ & $\phi_{\text{WORD}}(\delta,\rho)$\\[0.5ex]
%\hline %inserts single line
%\end{tabular}
...
$\mu{\textsc{word}}(\delta,\rho)$ & 
%$\lambda, \sigma, \langle  \Phi_{J} \dots \Phi_{2}, \Phi_{1}, x \rangle$  
$\Phi{\textsc{word}}(\rho,\delta)$ \\
    				& & $\langle \mu_{I} \dots \mu_{2}, \mu_{1}, x \rangle$ & $\langle \Phi_{J} \dots \Phi_{2}, \Phi_{1}, x \rangle$ \\[0.5ex]
\hline 
\end{tabular}
\label{tab:morphreps} % is used to refer this table in the text
\caption{Three levels of representation} % title of Table
\end{table}
...
Now, suppose we were to make a copy of the morphomic representation 
$\mu{\textsc{word}}(\lambda,\sigma)$, and we were then to replace the copy's original $\sigma$ with $\tau$ such that $\sigma \ne \tau$. 
 This would change the copy's MPS, of course, but it would
 not necessarily change its surface (phonological) form. That is, 
 $\Phi{\textsc{word}}(\lambda,\tau)$ may be identical to 
 $\Phi{\textsc{word}}(\lambda,\sigma)$. This kind of identity is in fact evident in the English
 present-tense verb conjugation (see table ref{tab:engverbpres}).
 All non-3.sg present-tense English verbs are realized as the bare form of the verb.
...
That is, suppose $\lambda = \textsc{kick}$, 
$\sigma =$ $\{\text{\textsc{pers}:1, \textsc{num}:pl,  \textsc{tns}:pres} \}$, 
and $\tau =$ $\{\text{\textsc{pers}:2, \textsc{num}:sg, \textsc{tns}:pres}\}$.
%Suppose also that for both MPSs ($\sigma$ and $\tau$), and that we are dealing 
%Then, $(\lambda, \sigma)$  and $(\lambda, \tau)$ would map 
%Then, $(\lambda, \sigma)$ and $(\lambda, \tau)$.
These are two different $\langle$ lexeme, MPS $\rangle$ pairs correspond to the \emph{same} phonological form:
$\Phi{\textsc{kick}}(\lambda,\sigma) = \text{kick}$, and 
$\Phi{\textsc{kick}}(\lambda,\tau) = \text{kick}$; 
It therefore must be the case that $\mu{\textsc{kick}}(\lambda,\sigma) = \mu{\textsc{kick}}(\lambda,\tau)$.
...
%But suppose our two word forms were $\phi_{\text{WORD}}(\lambda_1,\sigma)$ and $\phi_{\text{WORD}}(\lambda_2,\tau)$, where $\lambda_1 \ne \lambda_2$ and $\lambda_2 \ne \tau$. That is, we would be dealing with neither the same lexeme nor the same MPS. This would be a strange situation and moreover, one that would probably never occur. However, 
\cite{round:2015, round:md:2016} categorizes morphomes into three types, 
namely \emph{rhizomorphomes}, \emph{metamorphomes}, and \emph{meromorphomes}. 
Rhizomorphomes and metamorphomes both transcend particular lexemes; 
both encompass multiple lexemes. % and thus multiple paradigms.
More specifically, a rhizomorphome is a set of stems or roots (depending on the language in question) whose members all take the same set of inflectional exponents. 
% rhizomorphome.
...
A metamorphome is a pattern of stem equivalence that spans multiple paradigms lexemes (and thus multiple lexemes).
Because it occurs in multiple lexemes, the stem itself is different in
each paradigm, so it is not the stem itself that is the shared ``aspect of form" \cite{round:2015}.
Rather, it is the \emph{pattern} of paradigm cells that is shared. 
...
%when one
One good indicator of a metamorphome is that it is difficult to describe in prose. The L-morphome
is a class of verb lexemes in Romance languages such that the following is true: 
Let the cells of the present subjunctive sub-paradigm are arranged in a single 6-cell row, and
...
of the subjunctive cells, share the same stem, as illustrated This is illustrated in the table \ref{subtab:digo}
with respect to the lexeme \textsc{digo}. Notice the ``L" shape. Table \ref{subtab:crezco} shows
another``instantiation," as it were, of the same L-morphome morphome; i.e., the same cell pattern,
but for a different lexeme. Since the lexeme is different, the stem is different.
...
That is, it is the \emph{set} of verb 
stems that coincide with the following highly unnatural class of properties:
%\begin{equation}
%\{\text{\textsc{tns}:pres}, \text{\textsc{mood}:subj}\vee(\text{\textsc{mood}:ind}\wedge\text{\textsc{pers}:1}\wedge\text{\textsc{num}:sg})
%\end{equation}
\begin{equation}
\{ \text{\textsc{tns}:pres}, \text{\textsc{mood}:subj}\} \vee \{\text{\textsc{tns}:pres},\text{\textsc{mood}:ind},\text{\textsc{pers}:1},\text{\textsc{num}:sg}\}
\end{equation}
The stem that coincides with this MPS yields an $L$ shape when its verb forms are arranged 
...
lexeme. Different lexemes yield different \emph{instantiations} L-morphome stems.
...
%MPS}. For example, Romance languages exhibit what \cite{maiden:2005} calls \emph{L-morphomes}. An L-morphome is basically a stem-equivalence class. In particular, it is the set of stems that distinguish the verbs with following unnatural MPS: $\sigma = \{$\textsc{tns}:pres, \textsc{mood}:subj$\vee$(\textsc{mood}:ind$\wedge$\textsc{pers}:1$\wedge$\textsc{num}:sg)$\}$. This MPS yields an $L$ shape when its verb forms are arranged as in table \ref{tab:l-morphome}. This is a common pattern among Romance verb lexemes (cite).
...
\label{tab:l-morphome} % is used to refer this table in the text
\caption{The stem \textit{crezc-} `say' is an L-morphome in Spanish.}
\end{table}

Another example of a metamorphome is the English perfect participle, originally 
identified as a unified, albeit complex, morphome by \cite{aronoff:1994}. The term \emph{perfect participle} 
itself refers not to particular syntactic category, but 
to a class of 
forms whose members 
can serve certain syntactic purposes. For instance, it can be combined
...
Beard's Separation Hypothesis, namely that the form of any morphological exponent, whether derivational or inflectional,
is \emph{separate} from its meaning, has played important role in the development of autonomous
morphological theory. According to the Separation Hypothesis, the same phonological operation $\Phi_i$ can be used to realize 
different sets of morphosyntactic/derivational features.

Note the equivalence between the present-perfect-tense verb on the 
one hand and the passive verb on the other in each of the examples 
...
\label{ex:pastpart}
	...
	\ex Root ($\rho$): $\surd$\textsc{sing}
		...
		\end{xlist}
\end{exe}
\emph{Within} each of these three examples, i.e., from the perspective of a particular root, the perfect participle form is identical to the passive form. However,
from a hyper-root perspective, so to speak, we see that the precise \emph{manner} of the sameness varies from root to root.  
Table \ref{tab:engpastperf} summarizes five types of the $\mu{\textsc{perf}}$. But note that this table
...
%\textsc{buy} & bought & (has/had) bought & (was) bought \\ [1ex] % [1ex] adds vertical space
\hline 
\end{tabular}
\label{tab:engpastpart} % is used to refer this table in the text
\caption{The English past-participle: a \emph{polyvalent polysemous} morphome} % title of Table
\end{table}
...
\centering
  \subtable[broken\label{subtab:eng-break}]{
    \centering
       ...
       %\toprule
       %\hline
        %\multicolumn{3}{c}{\textipa{bro\'Uk\textbari n} `broken'}\\
        %\midrule
        \hline
    ...
    \centering
       ...
       %\toprule
       %\hline
        %\multicolumn{2}{c}{\textipa{s2N} `sung'}\\
       % \midrule
        \hline
...
%i.e., a lexeme and an MPS, to a phonologically realized surface form.
%\begin{equation}
...
%In particular, And it is impossible that identical surface forms should ever correspond to different morphomes. Meaning is irrelevant where morphomic mapping is concerned. Moreover, \emph{partial} identities between forms are also indicative of shared morphomic structure. These are strong claims. To find a morphome, according to Round, one need only find a common subsequence between two words that share the same lexeme, but may or may not share the same MPS. % a shared part, among a set of words. 
...
\caption[]{\cite{aronoff:md:2016} classifies morphomes according to their complexity. 
...
diagram is a reproduction of the one in \cite{aronoff:md:2016}.) } \label{fig:mtypes}
\end{figure}
...
highly (and idiosyncratically) fusional.  %Morphomic analysis offers  
Consider the forms in \ref.{tab:fusion}. 
...
In this section, we shall be primarily concerned with the
...
%\label{fig:t}
%\end{figure}
In Hebrew %nominals, i.e., nouns and adjectives, 
...
%feminine gender too to be attributed to random chance. 
...
%the inflectional (i.e., grammatical) \textit{t} in the feminine adjectives in\ref{subtab:der-adjectives} 
%derivational use of \textit{t} in the noun-deriving suffix \textit{-it}, as in 
%the derivation of  \textit{nagarit} `female carpenter'
%from \textit{nagar} `(male) carpenter' (see table \ref{subtab:der-nouns-i}). 
...
% accompanied by vowel accompanied by a range of vowels. 
...
%The \textit{-t} deletes in feminine singular absolute nouns. 
...
\vspace{7pt}
      \subtable[Nouns derived via \textit{-i}\label{subtab:der-nouns-i}]{
     \centering
    \begin{tabular}{l l l c} % creating 3 columns
   \toprule
    \textsc{base} &  \textsc{sg} &  \textsc{pl} & Gloss \\ 
    \midrule
    pax \textit{`tin'} & pax-it & pax-iy-ot & `cultured' \\
   ...
   \vspace{7pt}
   \subtable[Nouns derived via \textit{-u}\label{subtab:der-nouns-u}]{
     \centering
...
but \emph{roots.}
In DM, the term \emph{root} (along with \emph{formative}) is used to refer 
to maximally primitive units of meaning. (It is not to be confused with the 
consonantal root of Semitic morpology. Semitic roots
would roots would qualify as DM roots, but the DM root is more general than the Semitic root.)
...
Because \textit{-at} is a root, therefore, it %never attaches only to roots, 
...
%present. 
It is just phonological null, according to DM.
...
they
cannot be roots and are thus free of roots' selectional constraints.
...
Note that it is DM that motivates Faust's decision to posit a single 
...
%and thus could not be a DM root.  
...
a unified \emph{morphome} $\mu{\textsc{i}}$. Like Faust's expletive /-i/, %morphological unit /-i/.
...
\textit{-a} and \text{-t} both have the \emph{synchronic} underlying form /-at/.
...
\begin{table}
\begin{center}
   \subtable[{\textglotstop}{avavit} `spring-like'\label{subtab:heb-avivit}]{
     \centering
        \begin{tabular}{l c c c}
       \toprule
       ...
       Morphomic & $\mu$\textsc{a.i}, $\surd \textsc{\textipa{P}bb}$ & $\mu{\textsc{i}}$  & $\mu{\textsc{t}}$ \\ 
        Lexical & \multicolumn{3}{c}{$\surd \textsc{\textipa{P}bb}$, \{\text{adj}, \text{fem}, \text{sg}\}}\\
        \bottomrule 
    \end{tabular}
    }
%      \vspace{7pt}
...
%    }
    \vspace{7pt}
      \subtable[xalalit\label{subtab:heb-xalalit}]{
     \centering
    \begin{tabular}{l c c}
       \toprule
        ...
        \midrule
          \multirow{2}{*}{Phonological} & xalal & -it \\
           ...
           Morphomic & $\mu$\textsc{a.a}, $\surd$\textsc{xll} & $\mu$\textsc{i}, $\mu$\textsc{t} \\ 
            Lexical & \multicolumn{2}{c}{$\surd$\textsc{xll}, \{\text{noun}, \text{fem}, \text{sg}\}}\\
       \bottomrule 
    \end{tabular}
    }
%      \vspace{7pt}
...
%    \end{tabular}
          \vspace{7pt}
  \subtable[xalaliyot `spaceships'\label{subtab:heb-xalaliyot}]{
    \centering
        \begin{tabular}{l c c c }
       \toprule
       % //\textsc{noun base} &  \textsc{masc.sg} & \textsc{fem.sg} \\ [0.5ex]
        ...
        Morphomic & $\mu${\textsc{a.a}}, $\surd$\textsc{xll} & $\mu$\textsc{i}  &  $\mu$\textsc{o}, $\mu$\textsc{t} \\
                            %\multicolumn{2}{c}{$\mu$\textsc{o}, $\mu$\textsc{t}} \\ %\hline
        Lexical & \multicolumn{3}{c}{$\surd$\textsc{xll}, \{\text{noun}, \text{fem}, \text{pl}\}}\\
        \bottomrule 
    \end{tabular}
  }
...
%\begin{itemize}
%\item 
ULM systems learn do not learn morphemes. Because they do not have access to
...
able to discover facts \ref{ex:Xi} and \ref{ex:Yi} and from them deduce \ref{ex:i-and-t}: %and 2 and arrive at  
...
\ex Therefore, there must be, \textbf{in addition to} \textit{-i} and \textit{-t}, a distinct suffix \text{-it}
\end{exe}
...
%on a ULM system's use context. Even so, facts \ref{ex:YequalsXi} and \ref{ex:YneXi} are 
%significant observations. 

It would be difficult, however, for most ULM systems to capture such subtlety. Most ULM systems would probably just decompose \textit{-it} 
...
% as the the phonological forms \textit{-t}, \textit{-i}, and \textit{-it} are made uare only two morphomes present in this case, 
 as there are only two morphomes at work in generating \textit{-t}, \textit{-i}, and \textit{-it}, namely $\mu$\textsc{i} and $\mu$\textsc{t} 
It would thus depend on how one thinks of the of output subsequences. One could argue that while they are composed of surface-level characters,
...
from (REF) that an MCMM can map multiple causes (multiple morphological units)
 to a single surface-level node. Thus, if Multimorph's MCMM finds 
 \textit{-t}, \textit{-t}, \text{-it} as three distinct
 ...
 But if it should map multiple clusters to individual surface-level nodes, it would not do so in a hierarchical way; \textit{-t}, \textit{-t}, \text{-it} would
...
morphosyntactic property sets. %Multimorph
...
%	What about \emph{metamorphomes} (and rhizomorphomes)? 
%	Metamorphomes, like the Romance L-morphome, the English
%	${\mu}\textsc{perf}$ morphome, and the vowel patterns of the Hebrew binyanim, 
%	involve potentially many lexemes. A paradigm-related pattern
%	must be instantiated multiple times---i.e., by multiple lexemes---in order
%	to be considered a pattern. One might describe such morphomes 
%	as \emph{extra-lexemic}
	 
	%Multimorph is capable of learning the dig- and crezc-, but not the identity between dig- and crezc-.
	Metamorphomes involve two kinds of identity simultaneously. These are \emph{intra-}paradigm and {inter-}paradigm identity: 
	\begin{enumerate}
		...
		%The full set of cells that are so related--the cells, is called the \emph{pattern of intra-paradigm identity}.
		%Within the paradigm of a \emph{particular} lexeme, certain cells have stem $A$, e.g., dig-, whereas other forms have stem $B$, e.g., dec-. Cells that have the same The comparison is between specific forms, i.e., the particular phonemes of dig and dec.  
		%some aspect of form, usally the stem.--but not all---cells within a paradigm share the same stem (i.e., a particular meromorphome)
	...
	\end{enumerate} 
	%, and I surmise
	%they are beyond most unsupervised morphological learners. 
...
lexeme. 
% to a set of paradigm cells (not to the pieces of form that ``occupy" the cells, but to the cells themselves), The pattern is such that the same cells 
%(which are essentially just orderly combinations of morphosyntactic properties) 
%across different lexemes exhibit meromorphome sharing, though the particular meromorphome being shared is different for each lexeme.
% i.e., if multiple lexemes are realized  \cite{round:2015} 
%	Thus, this sort of morphome exists outside of the paradigm of an individual lexeme. 
%	But within a particular paradigm cell, we see \emph{meromorphomes}--the 
%	pieces of form that together compose a particular instance of a metamorphome. 
%	A metamorphome instance is 
%Multimorph can learn meromorphomes, but 
...
Recall from chapter (REF) that Multimorph's MCMM consists of
...
%The morphological units in Multimorph's output approximate meromorphomes
%in most cases. Or more precisely,
...
brand of autonomous morphology. It is thus appropriate that ULM systems--Multimorph
...
%The purpose of this subsection is to delineate what an MCMM can and 
%cannot learn 
...
%
%\item The very idea of ULM [implies] that there is an intermediate layer of organization between phonology and morphosyntax. 
%\end{itemize}
%
...
%
%Do other ULM approaches find morphomes? If so, how do the do it without morphosyntactic annotation?
