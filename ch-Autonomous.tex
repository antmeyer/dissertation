...
% A state of mismatch 
% between $\sigma$ and 
% $\sigma^\prime$ is called \emph{deponency}, which retains 
...
% is a condition wherein a content cell contains one morphosyntactic property 
% set, but is mapped to a form cell that is typically associated with a different 
...
%Another type of non-canonicity in content-to-form mappings is 
%A phenomenon called 
\emph{syncretism}, wherein the mapping from content to form cells is many to one. 
%is 
%a many-to-one mapping from content to form cells. 
...
\toprule
	& 1.sg & 2.sg & 3.sg & 1.pl & 2.pl & 3.pl \\ % inserts table
\cmidrule{2-7}
